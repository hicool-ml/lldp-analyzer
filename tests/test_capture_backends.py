...
from lldp.capture_backends import choose_backend, PCAPBackend, AFPacketBackend, HAS_PCAPY, HAS_DPKT
...
class TestAFPacketBackend:
    ...
    def test_requires_linux(self):
        ...
        if platform.system().lower() != "linux":
            with pytest.raises(RuntimeError, match="AFPacketBackend is supported only on Linux"):
    ...
    def test_friendly_permission_error_message(self):
        ...
        with patch('socket.socket') as mock_socket:
            ...
            assert "setcap cap_net_raw+ep" in error_msg or "raw socket privileges" in error_msg
...
class TestFastPathFiltering:
    ...
    def test_cdp_dst_mac_passes(self):
        ...
        cdp_packet = b'\x01\x00\x0c\xcc\xcc\xcc' + bytes(7)  # CDP dst MAC + rest of header
    ...
    def test_non_target_packet_filtered(self):
        ...
        should_filter = (
            ethertype not in (b'\x88\xcc', b'\x20\x00') and
